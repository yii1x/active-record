 ...
 * about this class.
 *
 * @property DbCriteria $dbCriteria The query criteria that is associated with this model.
 * This criteria is mainly used by {@link scopes named scope} feature to accumulate
 * different criteria specifications.
 * @property ActiveRecordMetaData $metaData The meta for this AR class.
 * @property DbConnection $dbConnection The database connection used by active record.
 * @property DbTableSchema $tableSchema The metadata of the table that this AR belongs to.
 * @property DbCommandBuilder $commandBuilder The command builder used by this AR.
...
{
    const string BELONGS_TO = BelongsToRelation::class;
    const string HAS_ONE = HasOneRelation::class;
    const string HAS_MANY = HasManyRelation::class;
    const string MANY_MANY = ManyManyRelation::class;
    const string STAT = StatRelation::class;

    /**
     * @var DbConnection the default database connection for all active record classes.
    ...
    private bool $_new = false;                        // whether this instance is new or not
    private array $_attributes = [];                // attribute name => attribute value
    private array $_related = [];                    // attribute name => related objects
    private $_c;                                // query criteria (used by finder only)
    private $_pk;                                // old primary key value
    private string $_alias = 't';                        // the table alias being used for query
     ...
     * This is a shortcut method to {@link DbConnection::cache()}.
     * It changes the query caching parameter of the {@link dbConnection} instance.
     * @param integer $duration the number of seconds that query results may remain valid in cache.
     * If this is 0, the caching will be disabled.
     * @param CacheInterface $dependency the dependency that will be used when saving
     ...
     * @throws DbException if the relation is not specified in {@link relations}.
     */
    public function getRelated($name, $refresh = false, $params = [])
    {
        ...
        if (!isset($md->relations[$name])) {
            throw new DbException(sprintf('%s does not have relation "%s".', get_class($this), $name));
        }
        ...
        if ($params !== []) { // dynamic query
            ...
            if ($params instanceof DbCriteria)
     ...
     * @since 1.1.2
     */
    public function resetScope(bool $resetDefault = true): static
    {
        if ($resetDefault)
     ...
     * available table schema. Make sure you have called {@link DbSchema::refresh}
    ...
    public function tableName(): string
    {
        ...
        if ($tableAttribute = current($reflection->getAttributes(Table::class))) {
            return $tableAttribute->newInstance()->name;
        }
     ...
     * @return DbConnection the database connection used by active record.
     * @throws DbException if "db" application component is not defined
     */
     ...
     * @return DbTableSchema the metadata of the table that this AR belongs to
     */
     ...
     * @return DbCommandBuilder the command builder used by this AR
     */
     ...
     * @see hasAttribute
     */
    public function setAttribute(string $name, mixed $value): bool
    {
        if (property_exists($this, $name))
            $this->$name = $value;
        elseif (isset($this->getMetaData()->columns[$name]))
    ...
    /**
     * Do not call this method. This method is used internally by {@link ActiveFinder} to populate
     ...
     */
    public function addRelatedRecord(string $name, mixed $record, mixed $index): void
    {
        if ($index !== false) {
            if (!isset($this->_related[$name]))
                $this->_related[$name] = [];
     ...
     */
    public function onBeforeSave(ModelEvent $event): void
    {
     ...
     * @param ModelEvent $event the event parameter
     */
    public function onBeforeDelete(ModelEvent $event): void
    {
     ...
     * @see beforeFind
     */
    public function onBeforeFind(ModelEvent $event): void
    {
     ...
     * @return ActiveFinder active finder for the operation
     *
     * @since 1.1.14
     */
    public function getActiveFinder(mixed $with): ActiveFinder
    {
     ...
     * @since 1.1.14
     */
    public function onBeforeCount(Event $event): void
    {
     ...
     * @since 1.1.14
     */
    protected function beforeCount(): void
    {
        if ($this->hasEventHandler('onBeforeCount'))
            $this->onBeforeCount(new Event($this));
    }
    ...
    protected function afterFind(): void
    {
        if ($this->hasEventHandler('onAfterFind'))
            $this->onAfterFind(new Event($this));
    }
     ...
     */
    public function beforeFindInternal(): void
    {
        $this->beforeFind();
    }
     ...
     * This method is internally used.
     */
    public function afterFindInternal(): void
    {
        $this->afterFind();
    }
     ...
     */
    public function insert($attributes = null): bool
    {
     ...
     * @throws DbException if the record is new
     */
     ...
     * @throws DbException if the record is new
     */
    public function saveAttributes($attributes): bool
    {
        if (!$this->getIsNewRecord()) {
            $values = [];
     ...
     * @since 1.1.8
     */
    public function saveCounters(array $counters): bool
    {
        $builder = $this->getCommandBuilder();
        $table = $this->getTableSchema();
     ...
     */
    public function equals(ActiveRecord $record): bool
    {
     ...
     * @since 1.1.0
     */
    public function getOldPrimaryKey(): mixed
    {
        return $this->_pk;
    }
     ...
     * @since 1.1.3
     */
    public function setOldPrimaryKey(mixed $value): void
    {
        $this->_pk = $value;
    }
     ...
     * @since 1.1.7
     */
    protected function query(DbCriteria $criteria, bool $all = false): mixed
    {
        $this->beforeFind();
     ...
     */
    public function applyScopes(DbCriteria &$criteria): void
    {
     ...
     * @since 1.1.1
     */
    public function getTableAlias(bool $quote = false, bool $checkScopes = true): string
    {
     ...
     * @param mixed|string $condition query condition or criteria.
     ...
     */
    public function find(mixed $condition = '', array $params = []): null|static
    {
     ...
     */
    public function findAll(mixed $condition = '', array $params = []): array
    {
        $criteria = $this->getCommandBuilder()->createCriteria($condition, $params);
     ...
     */
    public function findByPk(mixed $pk, mixed $condition = '', array $params = []): null|static
    {
     ...
     */
    public function findAllByPk(mixed $pk, mixed $condition = '', array $params = []): array
    {
        $prefix = $this->getTableAlias(true) . '.';
        $criteria = $this->getCommandBuilder()->createPkCriteria($this->getTableSchema(), $pk, $condition, $params, $prefix);
        return $this->query($criteria, true);
     ...
     * @return static|null the record found. Null if none is found.
     */
    public function findByAttributes(array $attributes, mixed $condition = '', array $params = []): null|static
    {
        $prefix = $this->getTableAlias(true) . '.';
     ...
     * @return static[] the records found. An empty array is returned if none is found.
     */
    public function findAllByAttributes(array $attributes, mixed $condition = '', array $params = []): array
    {
        $prefix = $this->getTableAlias(true) . '.';
        $criteria = $this->getCommandBuilder()->createColumnCriteria($this->getTableSchema(), $attributes, $condition, $params, $prefix);
        return $this->query($criteria, true);
     ...
     */
    public function count(mixed $condition = '', array $params = [])
    {
        $this->beforeCount();
        $builder = $this->getCommandBuilder();
     ...
     * @since 1.1.4
     */
    public function countByAttributes(array $attributes, mixed $condition = '', array $params = []): string
    {
        $prefix = $this->getTableAlias(true) . '.';
        $builder = $this->getCommandBuilder();
        $this->beforeCount();
     ...
     * This is equivalent to calling {@link DbCommand::queryScalar} with the specified
     ...
     */
    public function countBySql(string $sql, array $params = [])
    {
        $this->beforeCount();
     ...
     */
    public function exists(mixed $condition = '', array $params = []): bool
    {
        $builder = $this->getCommandBuilder();
        $criteria = $builder->createCriteria($condition, $params);
        $table = $this->getTableSchema();
    ...
    /**
     * Sets {@link DbCriteria::together} property to be true.
     * This is only used in relational AR query. Please refer to {@link DbCriteria::together}
     * for more details.
     * @return static the AR object itself
     * @since 1.1.4
     */
     ...
     */
    public function updateByPk(mixed $pk, array $attributes, mixed $condition = '', array $params = []): int
    {
        $builder = $this->getCommandBuilder();
        $table = $this->getTableSchema();
     ...
     * @return integer the number of rows being updated
     */
    public function updateAll(array $attributes, mixed $condition = '', array $params = []): int
    {
        $builder = $this->getCommandBuilder();
        $criteria = $builder->createCriteria($condition, $params);
     ...
     * @see saveCounters
     */
    public function updateCounters(array $counters, mixed $condition = '', array $params = []): int
    {
        $builder = $this->getCommandBuilder();
        $criteria = $builder->createCriteria($condition, $params);
        $command = $builder->createUpdateCounterCommand($this->getTableSchema(), $counters, $criteria);
     ...
     */
    public function deleteByPk(mixed $pk, mixed $condition = '', array $params = []): int
    {
        $builder = $this->getCommandBuilder();
     ...
     * @return integer the number of rows deleted
     */
    public function deleteAll(mixed $condition = '', array $params = []): int
    {
        $builder = $this->getCommandBuilder();
        $criteria = $builder->createCriteria($condition, $params);
        $command = $builder->createDeleteCommand($this->getTableSchema(), $criteria);
     ...
     */
    public function deleteAllByAttributes(array $attributes, mixed $condition = '', array $params = []): int
    {
        $builder = $this->getCommandBuilder();
        $table = $this->getTableSchema();
     ...
     * @param string|null $index the name of the attribute whose value will be used as indexes of the query result array.
     * If null, it means the array will be indexed by zero-based integers.
     * @return static[] list of active records.
     */
    public function populateRecords(array $data, bool $callAfterFind = true, $index = null): array
    {
        $records = [];
     ...
     */
    protected function instantiate(array $attributes): static
    {
