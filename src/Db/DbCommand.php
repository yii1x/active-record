<?php
/**
 * This file contains the DbCommand class.
 *
 ...
 *
 * It is usually created by calling {@link DbConnection::createCommand}.
 ...
 * can be accessed via the returned {@link DbDataReader}.
 *
 ...
 * @property DbConnection $connection The connection associated with this command.
...
class DbCommand
{
    ...
    private array $_fetchMode = [\PDO::FETCH_ASSOC];

    /**
     * Constructor.
     * @param DbConnection $connection the database connection
     * @param mixed|null $query the DB query to be executed. This can be either
     ...
     * for more details.
     */
    public function __construct(private readonly DbConnection $_connection, mixed $query = null)
    {
        if (is_array($query)) {
     ...
     * @since 1.1.7
     */
    public function setFetchMode(mixed $mode): static
    {
     ...
     * automatically.
     * @throws DbException if CDbCommand failed to prepare the SQL statement
     */
     ...
     * @param integer|null $dataType SQL data type of the parameter. If null, the type is determined by the PHP type of the value.
     * @param integer|null $length length of the data type
     ...
     */
    public function bindParam(mixed $name, mixed &$value, ?int $dataType = null, ?int $length = null, mixed $driverOptions = null): static
    {
        $this->prepare();
        if ($dataType === null)
     ...
     */
    public function bindValue(mixed $name, mixed $value, ?int $dataType = null): static
    {
        $this->prepare();
        if ($dataType === null)
    ...
    public function execute(array $params = []): int
    {
        ...
        try {
            ...
            throw new DbException(sprintf('CDbCommand failed to execute the SQL statement: %s', $message), (int)$e->getCode(), $errorInfo);
        }
    }
     ...
     * Please also note that all values are treated as strings in this case, if you need them to be handled as
     * their real data types, you have to use {@link bindParam} or {@link bindValue} instead.
     * @return DbDataReader the reader object for fetching the query result
     * @throws DbException execution failed
     */
     ...
     * @throws DbException execution failed
     */
    public function queryAll(bool $fetchAssociative = true, array $params = []): array|DbDataReader
    {
     ...
     * @throws DbException execution failed
     */
    public function queryRow(bool $fetchAssociative = true, array $params = array()): mixed
    {
     ...
     * @throws DbException execution failed
     */
    public function queryScalar(array $params = []): mixed
    {
     ...
     * @throws DbException if CDbCommand failed to execute the SQL statement
     */
    private function queryInternal(string $method, mixed $mode, array $params = []): mixed
    {
        ...
        try {
            ...
            throw new DbException(
                sprintf('CDbCommand failed to execute the SQL statement: %s', $message),
                (int)$e->getCode(),
                $errorInfo,
            );
        }
     ...
     * @throws DbException if "from" key is not present in given query parameter
     * @since 1.1.6
     */
     ...
     * @param mixed|string $columns the columns to be selected. Defaults to '*', meaning all columns.
     ...
     * @since 1.1.6
     */
    public function select(mixed $columns = '*', string $option = ''): static
    {
     ...
     * @since 1.1.6
     */
    public function setSelect(mixed $value): void
    {
        $this->select($value);
    }
     ...
     * @param mixed|string $columns the columns to be selected. See {@link select} for more details.
     * @return DbCommand the command object itself
     * @since 1.1.6
     */
    public function selectDistinct(mixed $columns = '*'): static
    {
     ...
     * @since 1.1.6
     */
    public function where(mixed $conditions, array $params = []): static
    {
     ...
     * @since 1.1.13
     */
    public function andWhere(mixed $conditions, array $params = []): static
    {
     ...
     * @since 1.1.13
     */
    public function orWhere(mixed $conditions, array $params = []): static
    {
        if (isset($this->_query['where']))
     ...
     * @since 1.1.6
     */
    public function setWhere(mixed $value): void
    {
        $this->where($value);
    }
     ...
     * @since 1.1.6
     */
    public function join(string $table, mixed $conditions, array $params = []): static
    {
     ...
     * @since 1.1.6
     */
    public function setJoin(mixed $value): void
    {
     ...
     * @since 1.1.6
     */
    public function leftJoin(string $table, mixed $conditions, array $params = []): static
    {
     ...
     * @since 1.1.6
     */
    public function rightJoin(string $table, mixed $conditions, array $params = []): static
    {
     ...
     * @since 1.1.16
     */
    public function naturalLeftJoin(string $table): static
    {
     ...
     * @since 1.1.16
     */
    public function naturalRightJoin(string $table): static
    {
     ...
     * @since 1.1.6
     */
    public function setGroup(mixed $value): void
    {
        $this->group($value);
    }
     ...
     * @since 1.1.6
     */
    public function having(mixed $conditions, array $params = []): static
    {
     ...
     * @since 1.1.6
     */
    public function setHaving(mixed $value): void
    {
        $this->having($value);
    }
     ...
     * @since 1.1.6
     */
    public function setOrder(mixed $value): static
    {
        $this->order($value);
        return $this;
    }
     ...
     * @since 1.1.6
     */
    public function setUnion(mixed $value): static
    {
     ...
     * @param mixed|string $conditions the conditions that will be put in the WHERE part. Please
     ...
     * @since 1.1.6
     */
    public function update(string $table, array $columns, mixed $conditions = '', array $params = []): int
    {
        $lines = array();
        foreach ($columns as $name => $value) {
            if ($value instanceof DbExpression) {
     ...
     * @since 1.1.6
     */
    public function delete(string $table, mixed $conditions = '', array $params = []): int
    {
     ...
     * @since 1.1.6
     */
    public function createTable(string $table, array $columns, ?string $options = null): int
    {
     ...
     * @since 1.1.6
     */
    public function renameTable(string $table, string $newName): int
    {
     ...
     * @since 1.1.6
     */
    public function addColumn(string $table, string $column, string $type): int
    {
     ...
     * @since 1.1.6
     */
    public function renameColumn(string $table, string $name, string $newName): int
    {
     ...
     * @since 1.1.6
     */
    public function alterColumn(string $table, string $column, string $type): int
    {
     ...
     * @param array|string $columns the name of the column to that the constraint will be added on. If there are multiple columns, separate them with commas or pass as an array of column names.
     * @param string $refTable the table that the foreign key references to.
     * @param array|string $refColumns the name of the column that the foreign key references to. If there are multiple columns, separate them with commas or pass as an array of column names.
     ...
     * @since 1.1.6
     */
    public function dropForeignKey(string $name, string $table): int
    {
     ...
     * @since 1.1.6
     */
    public function createIndex(string $name, string $table, array|string $columns, bool $unique = false): int
    {
     ...
     * @since 1.1.6
     */
    public function dropIndex(string $name, string $table): int
    {
     ...
     * @throws DbException if unknown operator is used
     */
    private function processConditions(mixed $conditions): string
    {
        ...
        throw new DbException(sprintf('Unknown operator "%s".', $operator));
    }
     ...
     * The method will automatically quote the table name unless it contains some parenthesis
     * (which means the table is given as a sub-query or DB expression).
     * @param mixed|string $conditions the join condition that should appear in the ON part.
     * Please refer to {@link where} on how to specify conditions.
     * @param array $params the parameters (name=>value) to be bound to the query
     * @return static the command object itself
     * @since 1.1.6
     */
    private function joinInternal(string $type, string $table, mixed $conditions = '', array $params = []): static
    {
        if (!str_contains($table, '(')) {
            if (preg_match('/^(.*?)(?i:\s+as|)\s+([^ ]+)$/', $table, $matches))  // with alias
     ...
     * @since 1.1.13
     */
    public function addPrimaryKey(string $name, string $table, array|string $columns): int
    {
