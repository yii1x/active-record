...
class DbCommandBuilder
{
     ...
     */
    public function __construct(DbSchema $schema)
    {
        $this->_schema = $schema;
     ...
     * @return DbCommand multiple insert command
     * @throws DbException if $data is empty.
     */
    protected function composeMultipleInsertCommand($table, array $data, array $templates = [])
    {
        if (empty($data))
            throw new DbException('Can not generate multiple insert command with empty data set.');
        ...
        foreach ($data as $rowKey => $rowData) {
            ...
            foreach ($columns as $columnName) {
                ...
                $columnValue = array_key_exists($columnName, $rowData) ? $rowData[$columnName] : new DbExpression('NULL');
                if ($columnValue instanceof DbExpression) {
     ...
     * @throws DbException if no columns are being updated for the given table
     */
    public function createUpdateCommand($table, $data, $criteria)
    {
        ...
        if ($fields === [])
            throw new DbException(sprintf('No columns are being updated for table "%s".', $table->name));
    ...
    public function createUpdateCounterCommand($table, $counters, $criteria)
    {
        ...
        if ($fields !== []) {
            ...
            throw new DbException(sprintf('No counter columns are being updated for table "%s".', $table->name));
    }
     ...
     * @return DbCommand the created command
     */
    public function createSqlCommand($sql, $params = []): DbCommand
    {
        $command = $this->_connection->createCommand($sql);
     ...
     * @param string|array $group GROUP BY
     * @return string SQL with GROUP BY.
     */
    public function applyGroup(string $sql, string|array $group): string
    {
        if ($group != '')
            return $sql . ' GROUP BY ' . implode(', ', (array)$group);
        else
            return $sql;
    }
     ...
     */
    public function applyHaving(string $sql, string|array $having): string
    {
        if ($having != '')
            return $sql . ' HAVING ' . implode(', ', (array)$having);
        else
            return $sql;
    }
     ...
     */
    public function createCriteria($condition = '', $params = [])
    {
     ...
     * @return DbCriteria the created query criteria
     */
    public function createPkCriteria($table, $pk, $condition = '', $params = [], $prefix = null)
    {
        $this->ensureTable($table);
        ...
        if (!is_array($pk)) // single key
            $pk = array($pk);
        if (is_array($table->primaryKey) && !isset($pk[0]) && $pk !== []) // single composite key
            $pk = array($pk);
     ...
     * @param string $prefix column prefix (ended with dot). If null, it will be the table name
     * @return DbCriteria the created query criteria
     * @throws DbException if specified column is not found in given table
     */
    public function createColumnCriteria($table, $columns, $condition = '', $params = [], $prefix = null)
    {
        ...
        foreach ($columns as $name => $value) {
            if (($column = $table->getColumn($name)) !== null) {
                ...
                throw new DbException(sprintf('Table "%s" does not have a column named "%s".', $table->name, $name));
        }
    ...
    public function createInCondition($table, $columnName, $values, $prefix = null)
    {
        ...
        if (is_string($columnName)) // simple key
        {
            if (!isset($table->columns[$columnName]))
                throw new DbException(sprintf('Table "%s" does not have a column named "%s".', $table->name, $columnName));
            ...
            foreach ($columnName as $name) {
                ...
                for ($i = 0; $i < $n; ++$i) {
                    if (isset($values[$i][$name])) {
                        ...
                        throw new DbException(sprintf('The value for the column "%s" is not supplied when querying the table "%s".', $name, $table->name));
                }
            }
            if (count($values) === 1) {
                $entries = [];
    ...
    protected function ensureTable(&$table)
    {
        if (is_string($table) && ($table = $this->_schema->getTable($tableName = $table)) === null)
            throw new DbException(sprintf('Table "%s" does not exist.', $tableName));
    }
