...
use Yii1x\ActiveRecord\Helpers\Util;
...
class DbCriteria
{
     ...
     * </ul>
     * The property value will be used as the parameter to the {@link ActiveRecord::with()} method
     * to perform the eager loading. Please refer to {@link ActiveRecord::with()} on how to specify this parameter.
     * @since 1.1.0
     */
    public $with;
    /**
    ...
    public function __wakeup()
    {
        ...
        if (is_array($this->params)) {
            foreach ($this->params as $name => $value) {
                if (str_starts_with($name, self::PARAM_PREFIX)) {
     ...
     */
    public function addCondition($condition, string $operator = 'AND'): static
    {
     ...
     * @return static the criteria object itself
     */
    public function addSearchCondition($column, $keyword, $escape = true, $operator = 'AND', $like = 'LIKE'): static
    {
        if ($keyword == '')
            return $this;
        if ($escape)
     ...
     * @return static the criteria object itself
     */
    public function addInCondition($column, $values, string $operator = 'AND'): static
    {
        if (($n = count($values)) < 1)
     ...
     * @since 1.1.1
     */
    public function addNotInCondition($column, $values, string $operator = 'AND'): static
    {
        if (($n = count($values)) < 1)
            return $this;
        if ($n === 1) {
     ...
     * @return static the criteria object itself
     */
    public function addColumnCondition($columns, string $columnOperator = 'AND', string $operator = 'AND'): static
    {
        $params = [];
     ...
     * @since 1.1.1
     */
    public function compare($column, $value, $partialMatch = false, string $operator = 'AND', bool $escape = true): static
    {
        if (is_array($value)) {
            if ($value === [])
                return $this;
     ...
     * @since 1.1.2
     */
    public function addBetweenCondition($column, $valueStart, $valueEnd, string $operator = 'AND'): static
    {
     ...
     * @param boolean|string $operator the operator used to concatenate where and having conditions. Defaults to 'AND'.
     * For backwards compatibility a boolean value can be passed:
     * - 'false' for 'OR'
     * - 'true' for 'AND'
     */
    public function mergeWith($criteria, bool|string $operator = 'AND'): void
    {
        if (is_bool($operator))
        ...
        elseif (!empty($criteria->with)) {
            ...
            foreach ((array)$criteria->with as $k => $v) {
                ...
                elseif (isset($this->with[$k])) {
                    ...
                    if (count($excludes) !== 0)
                        $this->with[$k] = Util::mergeArray($this->with[$k], $excludes);
                } else
                    $this->with[$k] = $v;
            }
        }
    ...
    {
        $result = [];
        foreach (['select', 'condition', 'params', 'limit', 'offset', 'order', 'group', 'join', 'having', 'distinct', 'scopes', 'with', 'alias', 'index', 'together'] as $name)
