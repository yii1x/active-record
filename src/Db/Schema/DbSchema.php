...
abstract class DbSchema
{
     ...
     */
    abstract protected function loadTable(string $name): ?DbTableSchema;

    /**
     * Constructor.
     * @param DbConnection $_connection
     */
    public function __construct(private DbConnection $_connection)
    {
        foreach ($_connection->schemaCachingExclude as $name)
     ...
     */
    public function getTableNames(string $schema = ''): array
    {
     ...
     */
    public function compareTableNames(string $name1, string $name2): bool
    {
     ...
     * @since 1.1
     */
    public function resetSequence(DbTableSchema $table, int|null $value = null)
    {
    }
     ...
     * @since 1.1
     */
    public function checkIntegrity(bool $check = true, string $schema = '')
    {
    }
     ...
     * If not empty, the returned table names will be prefixed with the schema name.
     * @return array all table names in the database.
     * @throws DbException if current schema does not support fetching all table names
     */
    protected function findTableNames($schema = '')
    {
        throw new DbException(Yii::t('yii', '{class} does not support fetching all table names.',
