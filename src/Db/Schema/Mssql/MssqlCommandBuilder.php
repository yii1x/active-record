...
namespace Yii1x\ActiveRecord\Db\Schema\Mssql;
 ...
 */
class MssqlCommandBuilder extends DbCommandBuilder
{
    /**
     * Creates a COUNT(*) command for a single table.
     * Override parent implementation to remove the order clause of criteria if it exists
     * @param DbTableSchema $table the table metadata
     ...
     * @throws DbException if no columns are being updated
     */
    public function createUpdateCommand($table, $data, $criteria)
    {
        ...
        if ($fields === array())
            throw new DbException(Yii::t('yii', 'No columns are being updated for table "{table}".',
     ...
     * Override parent implementation to check if an orderby clause if specified when querying with an offset
     * @param DbTableSchema $table the table metadata
     * @param DbCriteria $counters the query criteria
     ...
     * @param MssqlTableSchema $table table schema
     * @param DbCriteria $criteria criteria
     * @return DbCriteria the modified criteria
     */
