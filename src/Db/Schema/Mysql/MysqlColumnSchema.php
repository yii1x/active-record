 ...
 * @since 1.0
 */
class MysqlColumnSchema extends DbColumnSchema
{
    /**
     * Extracts the PHP type from DB type.
     * @param string $dbType DB type
     */
    protected function extractType(string $dbType): void
    {
        if (strncmp($dbType, 'enum', 4) === 0)
            $this->type = 'string';
        elseif (str_contains($dbType, 'float') || str_contains($dbType, 'double'))
            $this->type = 'double';
        elseif (str_contains($dbType, 'bool'))
            $this->type = 'boolean';
        elseif (str_starts_with($dbType, 'int') && !str_contains($dbType, 'unsigned') || preg_match('/(bit|tinyint|smallint|mediumint)/', $dbType))
            $this->type = 'integer';
        else
            $this->type = 'string';
    }
     ...
     */
    protected function extractLimit(string $dbType): void
    {
