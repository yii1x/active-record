...
use Yii1x\ActiveRecord\Db\Schema\{DbCriteria, DbTableSchema};
use Yii1x\ActiveRecord\Relations\{ActiveRelation, BelongsToRelation, HasManyRelation, HasOneRelation, ManyManyRelation};
...
class JoinElement
{
    ...
    public function lazyFind($baseRecord)
    {
        ...
        if ($child->relation instanceof HasManyRelation) {
     ...
     * @throws DbException if relation in active record class is not specified correctly
     */
    private function applyLazyCondition($query, $record)
    {
        ...
        if ($this->relation instanceof ManyManyRelation) {
            $query->conditions = [$this->relation->condition];
            ...
            foreach ($fks as $i => $fk) {
                ...
                if ($element->relation instanceof BelongsToRelation) {
                    if (is_int($i)) {
    ...
    public function buildQuery($query)
    {
        foreach ($this->children as $child) {
            if ($child->master !== null)
                $child->_joined = true;
            elseif ($child->relation instanceof HasOneRelation || $child->relation instanceof BelongsToRelation
        ...
        if (isset($this->records[$pk]))
            ...
            foreach ($this->children as $child) {
                if (!empty($child->relation->select))
                    $record->addRelatedRecord($child->relation->name, null, $child->relation instanceof HasManyRelation);
            }
     ...
     * @throws DbException if active record class is trying to select an invalid column
     */
        ...
        if ($select === '*') {
            ...
            foreach ($select as $name) {
                ...
                } elseif (preg_match('/^(.*?)\s+AS\s+(\w+)$/im', $name, $matches)) // if the column is already aliased
                {
                    ...
                    throw new DbException(
                        sprintf(
                            'Active record "%s" is trying to select an invalid column "%s". Note, the column must exist in the table or be an expression with alias.',
     ...
     * @param JoinElement $pke the join element contains primary keys
     ...
     * @param DbTableSchema $joinTable the join table
     * @param array $fks the foreign keys
     * @param JoinElement $parent the parent join element
     * @return string the join statement
     * @throws DbException if a foreign key is invalid
